...
def build_and_send(client, date_from, date_to, compare_from, compare_to, sections):
    ...
    if "summary" in sections:
        ...
        # Суммируем по всем каналам в summary_currency
        ...
        total_block = (
            ...
            f"├ Расход:     `{fmt_money(t['spend'], sym)}` ({fmt_delta(td(t['spend'], p_sum['spend']))}){fl('spend', td(t['spend'], p_sum['spend']))}\n"
            f"├ Конверсии:  `{t['leads']:.0f}` ({fmt_delta(td(t['leads'], p_sum['leads']))}){fl('leads', td(t['leads'], p_sum['leads']))}\n"
            f"├ CPA:        `{fmt_money(t_cpa, sym)}`\n"
            f"├ Клики:      `{int(t['clicks']):,}` ({fmt_delta(td(t['clicks'], p_sum['clicks']))}){fl('clicks', td(t['clicks'], p_sum['clicks']))}\n"
