...
def api_clients():
    ...
    for c in clients:
        ...
        result.append({"id": c["id"], "name": c["name"], "channels": active_channels})
...
def api_summary(client_id):
    ...
    for key, chan in ALL_CHANNELS.items():
        ...
        result.append({"channel": key, "channel_name": chan.name,
                        "channel_icon": chan.icon, "today": y, "compare": p})
...
def api_fetch():
    ...
    try:
        ...
        date_main    = data.get("date", "")
        date_compare = data.get("compare", "")
        if date_main:
            subprocess.Popen([PYTHON, "fetch_date.py", date_main], cwd=WORKDIR)
        if date_compare and date_compare != date_main:
            subprocess.Popen([PYTHON, "fetch_date.py", date_compare], cwd=WORKDIR)
        if not date_main and not date_compare:
            subprocess.Popen([PYTHON, "fetcher.py", "--no-telegram"], cwd=WORKDIR)
...
def api_send_telegram():
    if request.headers.get("X-Admin-Token", "") != ADMIN_TOKEN:
        abort(403)
    try:
        subprocess.Popen([PYTHON, "fetcher.py", "--telegram-only"], cwd=WORKDIR)
        return jsonify({"status": "started"})
    except Exception as e:
        return jsonify({"status": "error", "detail": str(e)}), 500
