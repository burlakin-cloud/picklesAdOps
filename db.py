...
def upsert_account(client_id: str, channel: str, date: str, m: dict):
    with get_conn() as conn:
        conn.execute("""
            INSERT OR REPLACE INTO account_metrics
                (client_id, channel, report_date, spend, impressions, clicks,
                 ctr, cpc, cpm, leads, cpa)
            VALUES (?,?,?,?,?,?,?,?,?,?,?)
...
def upsert_campaigns(client_id: str, channel: str, date: str, campaigns: list):
    with get_conn() as conn:
        conn.executemany("""
...
def get_account_metrics(client_id: str, date: str) -> list[dict]:
    with get_conn() as conn:
        rows = conn.execute("""
...
def get_campaign_metrics(client_id: str, channel: str, date: str) -> list[dict]:
    with get_conn() as conn:
        rows = conn.execute("""
...
def get_trend(client_id: str, channel: str, days: int = 7) -> list[dict]:
    with get_conn() as conn:
        rows = conn.execute("""
